...
fun main() {
    ...
    val pizzaChita = PizzaChita(90.0, 201.5, 115.5, 190.0)
...
private fun selectAddService(currentPizzaCity: PizzaCity) {
    if (currentPizzaCity is ProsmotrChacky) {
        currentPizzaCity.showCheckPhoto()
    }
    if (currentPizzaCity is Coffi) {
        currentPizzaCity.drinkSale()
    }
    if (currentPizzaCity is SousOpeca) {
        currentPizzaCity.offerSous()
    }
}